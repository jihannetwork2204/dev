...
app.listen(PORT, '0.0.0.0', () => {
    ...
    // Start Cloudflare Tunnel automatically when a tunnel token is available.
    // The local Node.js server keeps working even if cloudflared is unavailable.
    const TUNNEL_TOKEN = process.env.CLOUDFLARE_TUNNEL_TOKEN || process.env.TUNNEL_TOKEN || '';
    if (!TUNNEL_TOKEN) {
        console.log('[Cloudflare] Tunnel token not configured. Local server is running normally.');
        console.log('[Cloudflare] Set CLOUDFLARE_TUNNEL_TOKEN before starting to enable the tunnel.');
        return;
    }

    const binaryCandidates = process.platform === 'win32'
        ? [path.join(__dirname, 'cloudflared.exe'), path.join(__dirname, 'cloudflared')]
        : [path.join(__dirname, 'cloudflared'), 'cloudflared'];
    const binaryPath = binaryCandidates.find(p => p === 'cloudflared' || fs.existsSync(p));

    if (!binaryPath) {
        console.log('[Cloudflare Error] cloudflared binary not found.');
        console.log('[Cloudflare] Put cloudflared.exe (Windows) or cloudflared (Linux/macOS) beside server.js.');
        return;
    }

    console.log('[Cloudflare] Starting tunnel...');
    const cloudflared = spawn(binaryPath, ['tunnel', 'run', '--token', TUNNEL_TOKEN], {
        cwd: __dirname,
    ...
    cloudflared.on('error', err => {
        console.log(`[Cloudflare Error] Failed to start cloudflared: ${err.message}`);
    });
    cloudflared.on('exit', (code, signal) => {
        console.log(`[Cloudflare] Tunnel stopped (code=${code}, signal=${signal || 'none'}). Local server is still running.`);
    });
});
